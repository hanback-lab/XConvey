...
여기서 singnal() 등록은 프로그램에 외부 인터럽트 (Ctrl+c 와 같은 키입력)을 감지하기 위함입니다. Ctrl+c 키 입력이 감지되면 접속되어 있는 브로커 연결을 해제합니다. 여기서 on_disconnect() 가 호출되며 reconnect() 를 통해 브로커에 접속을 다시 시도합니다. 이때 접속은 즉시 시도하는것이 아니라 reconnect_delay_set() 에 설정한 5초 이후에 접속을 재시도합니다. 이 프로그램은 Ctrl+c 입력을 한번이 아니라 여러번 연달아 입력해야 프로그램이 종료됩니다. 
...
### JSON 메시지 
클라이언트로 발행 및 구독하는 정보가 장비 전체의 포토 센서일 때 이를 개별 토픽으로 분류하면 다음과 같습니다. 하지만 이 방법은 각 토픽마다 2바이트 크기의 센서값을 가지므로 발행자는 3번의 publish() 호출이 필요하고 구독자는 3번의 메시지 수신을 처리해야 합니다.
...
def publish_light(client):
    ...
    client.publish("xconvey/sensors/photo/value", json_data)
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        client.subscribe("xconvey/sensors/photo/value")
    else:
        print("connection fail:", rc)
        sys.exit(0)
