...
### 시리얼 통신 ID 정의 
시리얼 통신에 활용할 ID 목록을 정의합니다. 액추에이터 제어 ID, 센서 ID를 구분합니다.   
더불어, 수신된 MQTT 데이터를 프로토콜에 맞게 변환시켜줄 수 있는 룩업 테이블을 만들어 생성합니다. 
...
step_table = {'0':[0,0], '1':[0x5D,0xC0], '2':[0x6D,0x60], '3':[0x7D,0x00], '4':[0x8C,0xA0], '5':[0x9C,0x40], '6':[0xAB,0xE0], '7':[0xBB,0x80], '8':[0xCB,0x20], '9':[0xDA,0xC0],'10':[0xEA,0x60]} 
...
def _on_message(self, client, userdata, message):
    ...
    try:
        ...
        if message.topic.find('indicator') != -1 and message.topic.find('safety') != -1:
            ...
            serial_data.append(0x01)
            serial_data.extend(self.step_table[payload])
